...
/**
 * @author gyao
 * @create date 2019-04-30 3:53 PM
 **/
...
//            构造ShoppingItem实例，传入用户所编辑的商品和数量
            ShoppingItem shoppingItem = new ShoppingItem(product,
                    quantity);
            // request的getSession方法获取对象
            HttpSession session = request.getSession();
            // 调用HttpSession的getAttribute方法取回之前放入的对象
            List<ShoppingItem> cart = (List<ShoppingItem>) session
                    .getAttribute(CART_ATTRIBUTE);
            if (cart == null) {
                cart = new ArrayList<ShoppingItem>();
                // HttpSession的setAttribute方法将值放入HttpSession
